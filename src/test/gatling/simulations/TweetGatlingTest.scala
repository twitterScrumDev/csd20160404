...
class TweetGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Tweet entity")
        ...
        .repeat(2) {
            ...
            .post("/api/tweets")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "userid":"0", "publication_date":"2020-01-01T00:00:00.000Z", "tweet":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_tweet_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
